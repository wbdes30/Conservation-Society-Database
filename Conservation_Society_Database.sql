...
select t.TourID, t.TourName, 
	   concat('$', TourCost) "TourCost", 
       concat('$', TotalComponentCosts) "TotalComponentCosts", 
       concat('$', (TotalComponentCosts - TourCost)) "Savings" from Tour t, 
       (select pt.TourID, 
			   sum(cp.TourCost) "TotalComponentCosts" from Tour pt, 
               TourPackage tp, Tour cp where pt.TourID = tp.PackageTourID and tp.ComponentTourID = cp.TourID group by pt.TourID) TotalCosts 
		where t.TourID = TotalCosts.TourID;
